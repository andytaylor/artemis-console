...
import { Filter, ArtemisFilters } from './util/filter-util'
...
class ArtemisService {
    ...
    async createBrokerTopology(maxAddresses: number, addressFilter: string): Promise<BrokerTopology | null> {
     ...
     async filterQueuesForAddress(address: string, filters: Filter[]): Promise<Queue[]> {
        return new Promise<Queue[]>(async (resolve, reject)  => {
            ...
            const searchFilter: ArtemisFilters = {
    ...
    async getAddressesFiltered(page: number, perPage: number, artemisFilters: ArtemisFilters): Promise<string> {
    ...
    async getQueuesFiltered(page: number, perPage: number, artemisFilters: ArtemisFilters): Promise<string> {
